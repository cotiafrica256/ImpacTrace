...
class PublicationAdminController extends Controller {
 ...
 public function update(Request $r,Publication $publication){
  $d=$r->validate(['title'=>'sometimes|string|max:255','summary'=>'sometimes|string','content'=>'nullable|string','category'=>'nullable|string|max:100','cover_image'=>'sometimes|url|max:2048','youtube_url'=>'nullable|url|max:500','status'=>'sometimes|in:draft,review,approved']);
 ...
 public function publish(Request $r,Publication $publication){
  ...
  abort_unless(filter_var($publication->cover_image, FILTER_VALIDATE_URL),422,'A cover photo is required before publishing.');
