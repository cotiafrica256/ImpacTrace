...
class PublicPortalController extends Controller {
 public function index(Request $request){
    $q=Publication::query()->where('status','published')->whereNotNull('cover_image')->with(['packages'=>fn($x)=>$x->where('is_active',true)]);
 ...
 public function show(string $slug){
    return Publication::where('slug',$slug)->where('status','published')->whereNotNull('cover_image')->with(['packages'=>fn($x)=>$x->where('is_active',true)])->firstOrFail()->makeHidden(['content']);
 }
}