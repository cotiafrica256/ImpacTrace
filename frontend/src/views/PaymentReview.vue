<template>
  <section>
    <div class="flex flex-wrap items-end justify-between gap-4">
      ...
      <button @click="load" class="rounded-lg border px-4 py-2 text-sm">Refresh</button>
    </div>
...
const payments = ref([])
async function load () { const { data } = await api.get('/admin/payments/pending'); payments.value = data.data || data }
