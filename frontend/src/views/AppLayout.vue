    ...
    <aside :style="sidebarStyle" class="text-white md:min-h-screen md:w-72">
      ...
      <nav class="flex gap-1 overflow-x-auto p-2 md:flex-col md:gap-2 md:p-4">
        <RouterLink v-for="item in nav" :key="item.to" :to="item.to"
          class="rounded-xl px-4 py-3 text-sm font-medium text-slate-200 hover:bg-white/5 whitespace-nowrap"
          active-class="bg-white/10 text-[#d9b15d] ring-1 ring-white/10">
          {{ item.label }}
        </RouterLink>
      </nav>
...
const sidebarStyle = computed(() => ({
  background: `linear-gradient(180deg, ${currentOrg.value?.primary_color || '#0d1d2d'} 0%, ${currentOrg.value?.primary_color || '#0d1d2d'} 100%)`,
}))
...
const activeOrganizationName = computed(() => {
  ...
  return auth.user?.organization?.name || null
})

const roleLabel = computed(() => ({
  super_admin: 'COTIA Platform Admin', ed: 'Executive Director', meo: 'M&E Officer', po: 'Project Officer', fo: 'Field Officer',
}[auth.role] || auth.role))
...
watch(() => auth.selectedOrganizationId, (value) => {
  selectedOrgValue.value = value ? String(value) : ''
})
